...
const Dashboard: React.FC<DashboardProps> = ({ customers, transactions, onNavigate }) => {
  ...
  return (
    <div className="p-4 md:p-6 space-y-4 md:space-y-6 pb-24 md:pb-6 bg-surface dark:bg-darkSurface transition-colors duration-200 overflow-y-auto h-full overscroll-contain">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        <div className="bg-white dark:bg-darkCard p-3 md:p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
          <div className="flex items-center gap-2 text-primary mb-2">
            <Wallet size={18} className="text-red-500" />
            <span className="text-[10px] md:text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Outstanding</span>
          </div>
          <p className="text-xl md:text-2xl font-bold text-red-600 dark:text-red-400 truncate">₹{totalOutstanding.toLocaleString()}</p>
          <p className="text-[10px] md:text-xs text-gray-400 mt-1 font-medium">To Collect</p>
        </div>

        <div className="bg-white dark:bg-darkCard p-3 md:p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
          <div className="flex items-center gap-2 text-success mb-2">
            <TrendingUp size={18} className="text-green-500" />
            <span className="text-[10px] md:text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Received</span>
          </div>
          <p className="text-xl md:text-2xl font-bold text-green-600 dark:text-green-400 truncate">₹{totalCollected.toLocaleString()}</p>
          <p className="text-[10px] md:text-xs text-gray-400 dark:text-gray-400 mt-1">Cash In Hand</p>
        </div>

        <div className="bg-white dark:bg-darkCard p-3 md:p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
          <div className="flex items-center gap-2 text-blue-500 mb-2">
            <Users size={18} />
            <span className="text-[10px] md:text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Customers</span>
          </div>
          <p className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white truncate">{customers.length}</p>
          <p className="text-[10px] md:text-xs text-gray-500 dark:text-gray-400 mt-1">{activeCustomers} active</p>
        </div>
        
        <div className="bg-white dark:bg-darkCard p-3 md:p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
            <div className="flex items-center gap-2 text-pending mb-2">
                <AlertTriangle size={18} className="text-orange-500" />
                <span className="text-[10px] md:text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Overdue</span>
            </div>
            ...
            <p className="text-[10px] md:text-xs text-gray-500 dark:text-gray-400 mt-1">&gt; 30 days</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {/* Chart Section - Trend */}
          <div className="bg-white dark:bg-darkCard p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 h-64 md:h-80">
        ...
        <div className="divide-y divide-gray-100 dark:divide-gray-700">
            {transactions.slice(0, 5).map(t => (
                <div key={t.id} className="p-3 md:p-4 flex justify-between items-center hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                    ...
                    <span className={`font-bold ml-2 text-sm md:text-base ${t.type === TransactionType.BORROW ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`}>
